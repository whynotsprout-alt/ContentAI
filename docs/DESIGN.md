...
- `ModelConfiguration` 保留唯一 active 配置及不可变历史版本。每个版本完整保存 Base URL、API Key 密文、模型名、Temperature、上下文窗口、对话输出上限和结构化输出上限；API Key 只以 Fernet 密文、指纹和受限提示保存，读取接口不返回明文或密文。
