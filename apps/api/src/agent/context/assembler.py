    ...
    *,
    ...
    return "\n\n".join(
        [
            ...
            "当用户请求实时热点、搜索研究或其他必须依赖工具才能完成的结果时，必须在当前执行中"
            "立即调用相应工具并在取得结果后再回复。不得用“稍等一下”“稍后为你处理”或类似承诺"
            "结束回复，也不得声称正在、即将或已经执行一个实际没有调用的工具。",
            "工具调度由你基于完整对话语义自主判断。需要工具时，必须只输出平台原生的结构化"
            "tool call（含完整参数），不要输出任何解释、XML 标签、函数名文本或“我将调用工具”"
            "之类的旁白；不需要工具时才输出面向用户的普通回复。",
