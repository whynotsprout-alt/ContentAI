...
export interface AdminUser extends CurrentUser {
  ...
  chat_input_cost_usd: number;
  chat_output_cost_usd: number;
  chat_total_cost_usd: number;
  background_input_tokens: number;
  background_output_tokens: number;
  background_total_tokens: number;
  background_input_cost_usd: number;
  background_output_cost_usd: number;
  background_total_cost_usd: number;
  usage_call_count: number;
  completed_usage_call_count: number;
  missing_usage_call_count: number;
  failed_usage_call_count: number;
...
export const adminApi = {
  ...
  users: (params: { search?: string; status?: string; cursor?: string; limit?: number }, signal?: AbortSignal) => {
    const query = new URLSearchParams();
    if (params.search) query.set('search', params.search);
    if (params.status) query.set('status', params.status);
    if (params.cursor) query.set('cursor', params.cursor);
    query.set('limit', String(params.limit ?? 50));
    return request<AdminUserList>(`/api/admin/users?${query}`, { signal });
  },
  user: (userId: string, signal?: AbortSignal) => request<AdminUser>(`/api/admin/users/${userId}`, { signal }),
  ...
  sessions: (userId: string, cursor = '', limit = 50, signal?: AbortSignal) => {
  ...
  sessionDetail: (sessionId: string, signal?: AbortSignal) => request<AdminSessionSummary>(`/api/admin/sessions/${sessionId}`, { signal }),
  sessionMessages: (sessionId: string, cursor = '', limit = 200, signal?: AbortSignal) => {
    const query = new URLSearchParams({ limit: String(limit) });
    if (cursor) query.set('cursor', cursor);
  ...
  usage: (params: { group_by?: 'day' | 'model' | 'category'; user_id?: string } = {}, signal?: AbortSignal) => {
    const query = new URLSearchParams();
