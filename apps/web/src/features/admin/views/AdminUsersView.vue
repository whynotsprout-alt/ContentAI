...
import { ApiError, adminApi, type AdminSessionDetail, type AdminSessionSummary, type AdminUsageBucket, type AdminUser } from '@/shared/services/api';
...
let paginationRequestGeneration = 0;
...
function abort(controller: AbortController | null) {
  controller?.abort();
}
...
async function load() {
  ...
  try {
    const result = await adminApi.users(
      { search: search.value.trim(), status: status.value, cursor: currentCursor.value, limit: 50 },
...
async function selectUser(user: AdminUser) {
  ...
  error.value = '';
  try {
    const [userDetail, sessionResult, usageResult] = await Promise.all([
      adminApi.user(user.id, userController.signal),
      adminApi.sessions(user.id, '', 50, userController.signal),
      adminApi.usage({ user_id: user.id }, userController.signal)
    ]);
    if (generation !== userRequestGeneration || selected.value?.id !== user.id) return;
    selected.value = userDetail;
...
async function selectSession(session: AdminSessionSummary) {
  ...
  try {
    const [detail, messages] = await Promise.all([
      adminApi.sessionDetail(session.session_id, sessionController.signal),
      adminApi.sessionMessages(session.session_id, '', 200, sessionController.signal)
    ]);
    if (generation !== sessionRequestGeneration || selected.value?.id !== session.user_id) return;
    selectedSession.value = { ...detail, messages: messages.items };
    messagesNextCursor.value = messages.next_cursor;
  } catch (value) {
    if (generation === sessionRequestGeneration && !isAbortError(value)) error.value = parseError(value);
  } finally {
    if (generation === sessionRequestGeneration) sessionLoading.value = false;
  }
}
...
async function loadMoreSessions() {
  ...
  const generation = ++paginationRequestGeneration;
  abort(paginationController);
  paginationController = new AbortController();
  auditPaginationLoading.value = true;
  error.value = '';
  try {
    const result = await adminApi.sessions(userId, sessionsNextCursor.value, 50, paginationController.signal);
    if (generation !== paginationRequestGeneration || selected.value?.id !== userId) return;
    sessions.value.push(...result.items);
    sessionsNextCursor.value = result.next_cursor;
  } catch (value) {
    if (generation === paginationRequestGeneration && !isAbortError(value)) error.value = parseError(value);
  } finally {
    if (generation === paginationRequestGeneration) auditPaginationLoading.value = false;
  }
}
...
async function loadMoreMessages() {
  ...
  error.value = '';
  try {
    const result = await adminApi.sessionMessages(sessionId, messagesNextCursor.value, 200, paginationController.signal);
    if (generation !== paginationRequestGeneration || selectedSession.value?.session_id !== sessionId) return;
...
function formatCallCount(value: number | null | undefined) {
  return value == null || !Number.isFinite(value) ? '—' : formatTokens(value);
}
...
<template>
  <AdminShell title="用户" description="查看注册状态、数据规模、模型 Token 与费用消耗。">
      ...
      <div class="admin-workspace liquid-glass-strong">
        ...
        <aside class="admin-detail-panel" aria-label="用户详情" :aria-busy="userLoading">
          ...
          <template v-else>
            ...
            <section class="detail-token-breakdown" aria-labelledby="detail-token-title">
              <header><strong id="detail-token-title">Token 用量</strong><span>按调用用途拆分</span></header>
              <table class="detail-token-table">
                <caption class="sr-only">全部模型、对话和后台处理的 Token 用量</caption>
                <thead><tr><th scope="col">分类</th><th scope="col">输入</th><th scope="col">输出</th><th scope="col">总量</th></tr></thead>
                <tbody>
                  <tr><th scope="row">全部模型</th><td>{{ formatTokens(selected.input_tokens) }}</td><td>{{ formatTokens(selected.output_tokens) }}</td><td>{{ formatTokens(selected.total_tokens) }}</td></tr>
                  <tr><th scope="row">对话</th><td>{{ formatTokens(selected.chat_input_tokens) }}</td><td>{{ formatTokens(selected.chat_output_tokens) }}</td><td>{{ formatTokens(selected.chat_total_tokens) }}</td></tr>
                  <tr><th scope="row">后台处理</th><td>{{ formatTokens(selected.background_input_tokens) }}</td><td>{{ formatTokens(selected.background_output_tokens) }}</td><td>{{ formatTokens(selected.background_total_tokens) }}</td></tr>
                </tbody>
              </table>
            </section>
            <section class="detail-token-breakdown" aria-labelledby="detail-cost-title">
              <header><strong id="detail-cost-title">费用 (USD)</strong><span>按调用时单价换算</span></header>
              <table class="detail-token-table detail-cost-table">
                <caption class="sr-only">全部模型、对话和后台处理的美元费用</caption>
                <thead><tr><th scope="col">分类</th><th scope="col">输入</th><th scope="col">输出</th><th scope="col">合计</th></tr></thead>
                <tbody>
                  <tr><th scope="row">全部模型</th><td>{{ formatUsd(selected.input_cost_usd) }}</td><td>{{ formatUsd(selected.output_cost_usd) }}</td><td>{{ formatUsd(selected.total_cost_usd) }}</td></tr>
                  <tr><th scope="row">对话</th><td>{{ formatUsd(selected.chat_input_cost_usd) }}</td><td>{{ formatUsd(selected.chat_output_cost_usd) }}</td><td>{{ formatUsd(selected.chat_total_cost_usd) }}</td></tr>
                  <tr><th scope="row">后台处理</th><td>{{ formatUsd(selected.background_input_cost_usd) }}</td><td>{{ formatUsd(selected.background_output_cost_usd) }}</td><td>{{ formatUsd(selected.background_total_cost_usd) }}</td></tr>
                </tbody>
              </table>
            </section>
            ...
            <section v-if="usage.length" class="admin-detail-section"><header><strong>近期用量</strong></header><p v-for="bucket in usage.slice(0, 7)" :key="bucket.bucket" class="admin-usage-row"><span>{{ bucket.bucket }}</span><strong>{{ formatTokens(bucket.total_tokens) }} Token · {{ formatUsd(bucket.total_cost_usd) }}</strong></p></section>
    ...
    <AccessibleDialog v-if="auditOpen" overlay-class="admin-audit-dialog" title-id="audit-window-title" variant="fullscreen" @close="auditOpen = false">
