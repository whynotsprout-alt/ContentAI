...
import { createModelConfigRequestGuard } from '@/features/admin/modelConfigRequestGuard';
import {
  ApiError,
  adminApi,
...
const pricesValid = computed(() => [inputPrice.value, outputPrice.value].every((value) => {
...
function formatTemperature(value: number | null | undefined) {
  return value === null ? '自动（由模型决定）' : formatRuntimeNumber(value);
}

function formatPrice(value: number | null | undefined) {
  if (typeof value !== 'number' || !Number.isFinite(value)) return '—';
  return `$${new Intl.NumberFormat('en-US', { maximumFractionDigits: 6 }).format(value)} / 1M`;
}
  ...
  try {
    ...
    if (!options.preserveForm) {
      ...
      try {
        ...
        apiKey.value = '';
        temperatureMode.value = typeof result.temperature === 'number' ? 'custom' : 'auto';
        if (typeof result.temperature === 'number' && Number.isFinite(result.temperature)) {
          temperature.value = result.temperature;
        } else if (typeof temperature.value !== 'number' || !Number.isFinite(temperature.value)) {
          temperature.value = 0.2;
        }
        contextWindowTokens.value = result.context_window_tokens ?? 32_000;
        chatMaxTokens.value = result.chat_max_tokens ?? 8_000;
        structuredMaxTokens.value = result.structured_max_tokens ?? 8_000;
...
async function saveConfiguration() {
  ...
  const requestPayload: AdminModelUpdatePayload = {
    ...
    input_price_per_million_usd: Number(inputPrice.value),
    output_price_per_million_usd: Number(outputPrice.value),
  ...
  <AdminShell title="模型管理" description="配置全局 OpenAI 兼容端点，并为新任务选择统一模型。">
    ...
    <div class="model-admin-workspace" :aria-busy="loading || probing || saving">
      <section class="model-status-summary" aria-labelledby="model-status-summary-title">
        ...
        <dl v-if="!loading && !initialLoadFailed && configured" class="model-runtime-summary" aria-label="当前生效运行参数与价格">
          ...
          <div><dt>输入价格</dt><dd>{{ formatPrice(active?.input_price_per_million_usd) }}</dd></div>
          <div><dt>输出价格</dt><dd>{{ formatPrice(active?.output_price_per_million_usd) }}</dd></div>
        </dl>
      </section>
      ...
      <section class="model-config-panel" aria-labelledby="model-config-form-title">
        ...
        <form v-else class="model-config-form" @submit.prevent="saveConfiguration">
          ...
          <fieldset class="model-pricing-fields">
            <legend>价格配置</legend>
            <label class="model-field">
              <span>输入价格</span>
              <span class="model-price-control"><span>$</span><input v-model="inputPrice" type="number" min="0" step="0.000001" inputmode="decimal" required /><small>/ 1M Token</small></span>
            </label>
            <label class="model-field">
              <span>输出价格</span>
              <span class="model-price-control"><span>$</span><input v-model="outputPrice" type="number" min="0" step="0.000001" inputmode="decimal" required /><small>/ 1M Token</small></span>
            </label>
            <p>默认采用 Claude Opus 4.8 官方标准 API 价格：输入 $5、输出 $25 / 1M Token。保存后仅用于新 execution 的费用换算。</p>
          </fieldset>
      ...
      <details class="model-status-details">
        <summary>
          <span>查看完整生效状态</span>
          <small>版本、连接、运行参数、价格与验证信息</small>
        </summary>
