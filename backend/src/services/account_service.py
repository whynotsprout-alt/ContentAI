...
class AccountService:
    def list_accounts(self, session: Session, auth: AuthContext) -> list[AccountSummary]:
        ...
        return [self._to_summary(account) for account in rows]
    ...
    def get_account(self, session: Session, account_id: str, auth: AuthContext) -> AccountDetail:
        ...
        return self._to_detail(account)
    ...
    def delete_account(
        ...
        if self._account_has_references(session, account_id):
            raise AccountInUseError("Account has chat history or memories and cannot be deleted")
    ...
    def _account_has_references(session: Session, account_id: str) -> bool:
        if (
            session.exec(select(ChatSession.id).where(ChatSession.account_id == account_id).limit(1)).first()
            is not None
        ):
            return True

        if (
            session.exec(select(AgentInvocation.id).where(AgentInvocation.account_id == account_id).limit(1)).first()
            is not None
        ):
            return True
    ...
    @staticmethod
    def _get_account_for_auth(session: Session, account_id: str, auth: AuthContext) -> Account | None:
    ...
    def _to_detail(self, account: Account) -> AccountDetail:
        return AccountDetail(**self._to_summary(account).model_dump())
    ...
    def _raise_account_integrity_error(exc: IntegrityError) -> None:
        constraint_name = AccountService._extract_constraint_name(exc)
        if constraint_name == "ux_account_tenant_name":
            raise AccountAlreadyExistsError("Account name already exists in this tenant") from exc
        raise exc
